...
export class AppointmentsService {
    ...
    async getAllAppointments(dto: getAllAppointmentsDto) {
        try {
            if(dto.type === "doctor"){
                return await this.appointmentsRepository.findAll({where: {doctor_id: dto.id, }, include: {all:true}});
            }else if(dto.type === "doctor"){
                return await this.appointmentsRepository.findAll({where: {patient_id: dto.id,},include: {all:true}});
            }else{
    ...
    async getOneAppointment(appointments_id: number){
        try {
            ...
            if(!appointment){
                throw new HttpException({message: "appointment has no been found"}, HttpStatus.BAD_REQUEST)
            }
            return appointment
        }catch (e){
            throw new HttpException({message: e}, HttpStatus.INTERNAL_SERVER_ERROR)
